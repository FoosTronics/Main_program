...
if __name__ == "__main__":
    ...
    files = glob("C:\\Users\\kelvi\\Stichting Hogeschool Utrecht\\NLE - Documenten\\Test foto's\\ball\\*.png")

    files.sort()

    for file in files:  #check per file
        #code vanaf BallDetectionMain.py
        detect_ball = BallDetection(file)
        while True:
            detect_ball.getball_pos()
            if detect_ball.getFrame() is None:
                break
